...
if __name__ == '__main__':
    try:
        ...
        csrf_cookie = config.get('Session','csrf_cookie')
            ...
            for cnt, line in enumerate(sc):
                ...
                for item in response.text.split("\n"):
                    if strippedLine in item:
                        if "Invalid Code" in item:
                            print("Error code in line %d: %s"%(cnt+1, strippedLine))
                            # print("Error response: %s"%item.strip())
                            error.append(strippedLine)
                        elif "been redeemed":
                            print("Redeemed code in line %d: %s"%(cnt+1, strippedLine))
                            # print("Redeemed response: %s"%item.strip())
                        elif cnt/10 == 0:
                            print("Redeemed code %d"%(cnt))
            print("\n-------------------------------------------\n")
            print("Processed %d code"%cnt)
            if error:
                print("The following codes are not redeemed:")
                for code in error:
